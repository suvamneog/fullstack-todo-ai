...
const TaskItem = ({ todo, delButton, upperCaseOne, toggleComplete, index, copyToClipboard, editTodo }) => {
  ...
  return (
    <div
      className={`group flex flex-col sm:flex-row items-center gap-3 p-4 bg-gray-50 hover:bg-gray-200 rounded-lg transition-all transform ${
        isVisible
        ...
        <div className={`flex items-center gap-4 ${todo.completed ? 'text-gray-400' : 'text-gray-700'}`}>
          ...
          {isEditing ? (
            <input
              ...
              className="border-b border-gray-300 focus:outline-none focus:border-blue-500 px-1 py-0.5 text-xs sm:text-sm"
            />
          ) : (
            <span className={todo.completed ? 'line-through text-gray-400' : 'text-gray-500 font-bold text-xs sm:text-sm'}>
              {todo.task}
            </span>
          )}
        </div>
      </div>
      <div className="flex gap-2 mt-2 sm:mt-0 sm:gap-3">
        {isEditing ? (
          <button
            onClick={handleSave}
